...
import { Shield, Menu, Home, Settings, LogOut, Wrench, AlertCircle, Users, Calendar, CreditCard, Bell, Megaphone, Bed } from 'lucide-react';
    ...
    const fetchAnnouncements = async () => {
      if (user) {
        try {
          ...
          // Find the most recent active announcement that hasn't been shown
          const latestAnnouncement = fetchedAnnouncements
            .filter(a => a.status === 'active' && !a.isFirstTimeShown)
              ...
              <aside className="fixed top-0 left-0 w-[70%] h-full z-50 border-r bg-background">
                <nav className="space-y-1 p-4">
                  ...
                  <Link href="/student/sleepovers">
                    ...
                    >
                      <Bed className={`mr-2 h-4 w-4 ${isActiveLink('/student/sleepovers') ? 'text-white hover:text-black' : ''}`} />
                      Sleepovers
                    </Button>
                  </Link>
